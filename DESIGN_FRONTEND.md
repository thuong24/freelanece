...
- Typography scaling:
  - Hero: 40px (mobile) → 64px (desktop)
  - Section title: 28px → 40px
  - Body: 14px → 16px
...
- Components must:
  ...
  - Avoid overflow
  - Use flex + grid responsive utilities
...
FAIL if layout breaks on mobile.

## Implementation Rules

- MUST use Tailwind responsive classes (md:, lg:)
- DO NOT use fixed widths (px) unless necessary
- Prefer:
  - w-full, max-w-*
...
- No fixed height for containers
- No absolute positioning breaking layout
- No text overflow
